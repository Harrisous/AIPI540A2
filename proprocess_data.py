...
@retry(
    ...
    wait=wait_exponential(multiplier=1, min=2, max=10),  # Exponential backoff, 2-10 seconds
    retry=retry_if_exception_type((ConnectionError, TimeoutError)),  # Only retry network-related errors
    reraise=True,  # Re-raise exception on final failure
    before_sleep=lambda retry_state: print(f"Retrying {retry_state.attempt_number}/3 record {retry_state.args[0][0]}, waiting {retry_state.next_action.sleep} seconds...")
)
...
try:
    ...
    if len(unprocessed_df) > 0:
        print(f"Starting parallel processing of {len(unprocessed_df)} records using {NUM_WORKERS} worker threads...")
        # Prepare task list
        ...
        with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
            ...
            for future in as_completed(future_to_index):
                ...
                if pbar.n % 5 == 0 or pbar.n == len(tasks):  # Save every 5 records to reduce I/O operations
                    df.to_csv(TEMP_FILE, index=False)
